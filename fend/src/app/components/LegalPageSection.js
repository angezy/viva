...
function EditButton({ onClick, editable }) {
  ...
  return (
    <IconButton
      ...
      onClick={onClick}
      sx={{ position: "absolute", top: 12, right: 12, zIndex: 2, color: "#173a2b", bgcolor: "rgba(255,255,255,0.94)", "&:hover": { bgcolor: "white" } }}
    >
...
function Paragraphs({ text }) {
  return String(text || "")
    ...
    .filter(Boolean)
    .map((paragraph, index) => (
      <Typography key={index} sx={{ color: "#52645a", lineHeight: 1.85, mb: 2 }}>
        {paragraph}
      </Typography>
    ));
}
...
function LegalTable({ table }) {
  ...
  return (
    <Box sx={{ overflowX: "auto", mt: 2, mb: 2 }}>
      <Box component="table" sx={{ width: "100%", minWidth: 620, borderCollapse: "collapse", bgcolor: "white", border: "1px solid #dbe7dc" }}>
        <Box component="thead">
          <Box component="tr" sx={{ bgcolor: "#e4efe6" }}>
            {table.headers.map((header) => <Box component="th" scope="col" key={header} sx={{ textAlign: "left", p: 1.5, color: "#173a2b", fontSize: 13, fontWeight: 800, borderBottom: "1px solid #cbdccc" }}>{header}</Box>)}
          </Box>
        </Box>
        <Box component="tbody">
          {table.rows.map((row, rowIndex) => (
            <Box component="tr" key={rowIndex}>
              {row.map((cell, cellIndex) => <Box component="td" key={`${rowIndex}-${cellIndex}`} sx={{ p: 1.5, color: "#52645a", fontSize: 14, lineHeight: 1.5, borderBottom: "1px solid #e6eee7", verticalAlign: "top" }}>{cell}</Box>)}
            </Box>
          ))}
        </Box>
      </Box>
  ...
  return (
    <Box component="main" sx={{ bgcolor: "#f6f9f5", color: "#12372a", minHeight: "100vh", fontFamily: "'Space Grotesk','Segoe UI',sans-serif" }}>
      <Container maxWidth="md" sx={{ px: { xs: 2, md: 4 }, py: { xs: 4, md: 8 } }}>
        <Box component="header" sx={{ position: "relative", bgcolor: "#12372a", color: "white", borderRadius: { xs: 3, md: 4 }, p: { xs: 3, md: 6 }, mb: { xs: 5, md: 8 } }}>
          <EditButton onClick={onEdit.hero} editable={editable} />
          <Typography variant="overline" sx={{ color: "#a8d8b8", fontWeight: 800, letterSpacing: "0.14em" }}>{hero.eyebrow}</Typography>
          ...
          <Typography sx={{ color: "rgba(255,255,255,0.8)", lineHeight: 1.8, fontSize: { xs: "1rem", md: "1.1rem" }, maxWidth: 720 }}>{hero.intro}</Typography>
        </Box>

        <Stack spacing={{ xs: 4, md: 6 }}>
          {sections.map((section, index) => (
            <Box component="section" aria-labelledby={`legal-section-${index}`} key={`${section.title}-${index}`} sx={{ position: "relative", borderBottom: index === sections.length - 1 ? 0 : "1px solid #dbe7dc", pb: index === sections.length - 1 ? 0 : { xs: 3, md: 5 } }}>
              ...
              {Array.isArray(section.bullets) && section.bullets.length > 0 && (
                <Box component="ul" sx={{ color: "#52645a", pl: 3, mb: 2, "& li": { mb: 1.2, lineHeight: 1.7 } }}>
              ...
              {Array.isArray(section.steps) && section.steps.length > 0 && (
                <Box component="ol" sx={{ color: "#52645a", pl: 3, mb: 2, "& li": { mb: 1.2, lineHeight: 1.7, pl: 0.5 } }}>
        ...
        {(faqItems.length > 0 || (editable && onEdit.faq)) && (
          <Box component="section" aria-labelledby="legal-faq-title" sx={{ mt: { xs: 8, md: 11 }, position: "relative" }}>
            ...
            {faqItems.length === 0 && <Typography sx={{ color: "#52645a", py: 2 }}>No questions have been added yet.</Typography>}
            {faqItems.map((item, index) => (
              <Accordion key={`${item.question}-${index}`} expanded={expandedFaq === index} onChange={() => setExpandedFaq(expandedFaq === index ? -1 : index)} disableGutters elevation={0} sx={{ bgcolor: "transparent", borderTop: "1px solid #d7e3d8", "&:last-child": { borderBottom: "1px solid #d7e3d8" }, "&::before": { display: "none" } }}>
                ...
                <AccordionDetails id={`legal-faq-panel-${index}`} sx={{ px: 0, pt: 0, pb: 2.5 }}>
                  <Typography sx={{ color: "#52645a", lineHeight: 1.8 }}>{item.answer}</Typography>
                </AccordionDetails>
              </Accordion>
            ))}
          </Box>
        ...
        <Box sx={{ mt: { xs: 7, md: 10 }, p: { xs: 3, md: 4 }, bgcolor: "#e4efe6", borderRadius: 3 }}>
          <Typography component="h2" sx={{ fontWeight: 800, mb: 1 }}>Need help with an order?</Typography>
          <Typography sx={{ color: "#52645a", lineHeight: 1.7, mb: 2 }}>Contact Weluxo support if you need help understanding a policy or resolving an order question.</Typography>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={1.25}>
            <Button component={Link} href="/contact" variant="contained" endIcon={<ArrowForwardIcon />} sx={{ bgcolor: "#12372a", borderRadius: 999, textTransform: "none", fontWeight: 800 }}>Contact Support</Button>
            <Button component={Link} href="/shop" variant="outlined" endIcon={<ArrowForwardIcon />} sx={{ color: "#12372a", borderColor: "#8eaf96", borderRadius: 999, textTransform: "none", fontWeight: 800 }}>Continue Shopping</Button>
          </Stack>
        </Box>
      </Container>
    </Box>

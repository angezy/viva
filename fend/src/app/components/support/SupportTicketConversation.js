  ...
  return (
    <Box component="main" sx={{ bgcolor: "var(--color-background)", minHeight: "100vh", py: { xs: 3, md: 6 }, color: "var(--color-text-primary)", "& .MuiPaper-root": { borderColor: "var(--color-border)" }, "& .MuiPaper-root > .MuiBox-root:first-of-type": { bgcolor: "#ffffff", color: "var(--color-text-primary)" }, "& .MuiPaper-root > .MuiBox-root:first-of-type .MuiTypography-root": { color: "var(--color-text-primary)" }, "& .MuiPaper-root > .MuiBox-root:first-of-type .MuiTypography-overline": { color: "var(--color-accent)" }, "& .MuiPaper-root .MuiButton-root": { color: "var(--color-primary)" }, "& .MuiPaper-root .MuiButton-contained": { bgcolor: "var(--color-primary)", color: "#ffffff" } }}>
      <Container maxWidth="md">
        <Button component={Link} href="/account/support" startIcon={<ArrowBackIcon />} sx={{ mb: 2, color: "#12372a", textTransform: "none", fontWeight: 800 }}>My support tickets</Button>
        <Paper elevation={0} sx={{ border: "1px solid #dbe7dc", borderRadius: 3, overflow: "hidden" }}>
          <Box sx={{ bgcolor: "#0e2b20", color: "white", p: { xs: 2.5, md: 4 } }}>
            <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={2}>
              <Box>
                <Typography variant="overline" sx={{ color: "#a8d8b8", fontWeight: 800 }}>{ticket.ticketNumber}</Typography>
              ...
              <Chip label={ticket.status} sx={{ alignSelf: "flex-start", bgcolor: "#cbe8d2", color: "#12372a", fontWeight: 800 }} />
            </Stack>
          </Box>
          <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: "#fbfdfb" }}>
            {ticket.messages?.map((item) => <Box key={item.id} sx={{ mb: 2, display: "flex", justifyContent: item.senderType === "customer" ? "flex-end" : "flex-start" }}><Box sx={{ maxWidth: "85%", bgcolor: item.senderType === "customer" ? "#dcebe0" : "white", border: "1px solid #dbe7dc", borderRadius: 2.5, p: 2 }}><Typography sx={{ color: "#3e785e", fontWeight: 800, fontSize: 12, textTransform: "uppercase", mb: 1 }}>{item.senderType === "customer" ? "You" : item.senderType} · {new Date(item.createdAt).toLocaleString()}</Typography><Box sx={{ color: "#365345", lineHeight: 1.7, "& p": { mt: 0, mb: 1 }, "& ul": { pl: 3 } }} dangerouslySetInnerHTML={{ __html: item.contentHtml || `<p>${item.contentText || ""}</p>` }} />{item.attachments?.length > 0 && <Stack sx={{ mt: 1 }} spacing={0.5}>{item.attachments.map((file, index) => <a key={`${file.url || "attachment"}-${index}`} href={file.url} target="_blank" rel="noreferrer" style={{ color: "#3e785e", fontSize: 13 }}>{file.name || "Download attachment"}</a>)}</Stack>}</Box></Box>)}
          </Box>
          <Box component="form" onSubmit={send} sx={{ p: { xs: 2, md: 4 }, borderTop: "1px solid #dbe7dc" }}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", sm: "center" }} gap={1.5} sx={{ mt: 2 }}>
              <Stack direction="row" alignItems="center" gap={1} flexWrap="wrap">
                <Button component="label" variant="outlined" sx={{ borderRadius: 999, textTransform: "none", color: "#12372a", borderColor: "#8eaf96" }}>
                  Attach files
                ...
                {files.length > 0 && <Typography sx={{ color: "#52645a", fontSize: 12 }}>{files.map((file) => file.name).join(", ")}</Typography>}
              </Stack>
              <Button type="submit" disabled={sending} variant="contained" startIcon={<SendIcon />} sx={{ borderRadius: 999, bgcolor: "#12372a", textTransform: "none", fontWeight: 800 }}>{sending ? "Sending..." : "Send reply"}</Button>
            </Stack>

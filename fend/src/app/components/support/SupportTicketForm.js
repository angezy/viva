      ...
      <Container maxWidth="md">
        ...
        <Paper component="form" onSubmit={submit} elevation={0} sx={{ p: { xs: 2.5, md: 4 }, borderRadius: 3, border: "1px solid #dbe7dc", bgcolor: "white" }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}><TextField required fullWidth type="email" label="Email" value={form.email} onChange={(event) => update("email", event.target.value)} /></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth label="Order number" value={form.orderNumber} onChange={(event) => update("orderNumber", event.target.value)} /></Grid>
            <Grid item xs={12} sm={3}><TextField select fullWidth label="Category" value={form.category} onChange={(event) => update("category", event.target.value)}>{categories.map((item) => <MenuItem key={item} value={item}>{item}</MenuItem>)}</TextField></Grid>
